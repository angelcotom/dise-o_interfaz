...
def crear_cadena_Nveces():
    ...
    try:
        texto = input("Ingresa la palabra o cadena: ").strip()
        if not texto:
            print(" Error: La cadena no puede estar vacía.")
            return crear_cadena_Nveces()
        ...
        for i in range(1, repeticion + 1):
            print(f"{i}. {texto}")
        print("Cadena creada correctamente.")
    except ValueError:
...
def buscar_letra():
    ...
    cadena = input("Escribe una palabra o frase: ").lower().strip()
    letra = input("¿Qué letra quieres contar?: ").lower().strip()
